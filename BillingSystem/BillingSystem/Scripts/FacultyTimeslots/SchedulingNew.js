...
$(function () {
    ...
    $(".EmiratesMask").mask("999-9999-9999999-9");
    ...
    $('.searchHitMe').on('click', function (e) {//function to toggle previous visit list in & out
        //$('.searchSlide').toggleClass('moveLeft');
    });
    $('#spnPrevList').on('click', function (e) {//function to toggle previous visit list in & out
    ...
    $('#txtRecEveryDay, #txtRepeatMonthDay, #txtRecEveryWeekDays, #txtOVApptTypesFrequency').keypress(function (event) {
        //debugger;
        ...
        success: function (data) {
            if (data.IsError) {
                ...
                if (data.SkippedHolidaysData1.length > 0) {
                    msg = 'Records Saved Succesfully!, But some of the Physicians have Vacation or Appointments booked during selected date period.';
                } else {
...
var DeleteSchduling = function (epid, schid, schtype, extVal3) {
    ...
    switch (schtype) {
        case "1":
            jsonData = { eventParentId: schid == "" ? epid : "", schedulingId: schid, schedulingType: schtype, externalValue3: extVal3 };
            break;
        case "2":
...
function GetColorCodeStringAfterBind(data, phyName, RID, DPID, ETime, STime, PID, day) {
    ...
    if (day >= currentDate.getDate()) {
        retValue = '<div title="' + showTitle.replace(/<br>\s*$/, "") + '" class="color_td ' + showClass + '" attr-DeptId="' + DPID + '" attr-RoomId="' + RID + '" attr-Day="' + day + '"  attr-EndTime ="' + ETime + '" attr-StartTime ="' + STime + '" attr-PhysicianId ="' + PID + '"  onclick=OnClickGetData(this)>' + showData + '<div class="selected_column" style="display:none;"></div></div>';
    } else {
        retValue = '<div title="' + showTitle.replace(/<br>\s*$/, "") + '" class="color_td grey_column" attr-DeptId="' + DPID + '" attr-RoomId="' + RID + '" attr-Day="' + day + '"  attr-EndTime ="' + ETime + '" attr-StartTime ="' + STime + '" attr-PhysicianId ="' + PID + '"  >' + showData + '<div class="selected_column" style="display:none;"></div></div>';
    }
    return retValue;
}
function GetGolorCodeString(data, full, day) {
    ...
    if (data.indexOf("~") > -1) {
        ...
        showData = splitArray[0];
        showClass = GetShowDataValue(splitArray[1]);
        showTitle += "<div class='qtip_title'><h2>Physician: " + full.PhysicianName + "</h2><h2>Room: " + full.RID + "</h2></div>";
        for (var i = 2; i < splitArray.length; i++) {
            showTitle += "<div class='tooltip_row'>" + splitArray[i] + "</div>";
        }
    } else {
