...
namespace BillingSystem.Controllers
{
    public class DiagnosisController : BaseController
    {
        ...
        private readonly IDiagnosisCodeService _dcservice;
        ...
        public DiagnosisController(IBillActivityService blService, ICPTCodesService cptService, IEncounterService eService, IDiagnosisService service, IDiagnosisCodeService dcservice, IDRGCodesService drgservice, IFavoritesService favService, IOrderActivityService oaService, IPatientInfoService piService)
        {
        ...
        public ActionResult AdditionalDiagnosis(int? pId, int? eId)
        {
            ...
            var isPrimary = true;
            var isMajorCPT = true;
            var dModel = _service.GetNewDiagnosisByEncounterId(encounterId, patientId);
            var dList = _service.GetDiagnosisList(patientId, encounterId);
            ...
            var favDiagnosisList = _favService.GetFavoriteOrders(Helpers.GetLoggedInUserId(), Helpers.DefaultCptTableNumber, Helpers.DefaultDrgTableNumber, Helpers.DefaultHcPcsTableNumber, Helpers.DefaultDrugTableNumber, Helpers.DefaultServiceCodeTableNumber, Helpers.DefaultDiagnosisTableNumber);
            favDiagnosisList =
        ...
        public JsonResult GetDiagnosisCodes(string text, List<string> diagCode)
        {
            if (!string.IsNullOrEmpty(text))
            {
                ...
                var list = diagCode == null
                    ? _service.GetFilteredDiagnosisCodes(text, userId, Helpers.GetDefaultFacilityId(), Helpers.DefaultDiagnosisTableNumber)
                    : _service.GetFilteredDiagnosisCodes(text, userId, Helpers.GetDefaultFacilityId(), Helpers.DefaultDiagnosisTableNumber)
        ...
        public ActionResult GetList(string patientId)
        {
            ...
            {
                diagnosisList = _service.GetDiagnosisList(Convert.ToInt32(patientId), encounterobj.EncounterID);
            }
        ...
        {
            var diagnosisList = _service.GetDiagnosisList(Convert.ToInt32(patientId), Convert.ToInt32(encounterId));
        ...
        public JsonResult DeleteDiagnosis(int id)
        {
            ...
            var result = _service.DeleteCurrentDiagnosis(Helpers.GetLoggedInUserId(), id, Helpers.DefaultDrgTableNumber);
        ...
        {
            var list = _service.GetDiagnosisList(Convert.ToInt32(patientId));
        ...
        public ActionResult AddDiagnosisById(string id)
        {
            ...
            var drg = _drgservice.GetDrgCodesById(Convert.ToInt32(diagnosis.DRGCodeID));
        ...
        {
            var current = _service.GetDiagnosisList(Convert.ToInt32(vm.PatientID),
        ...
        public JsonResult GetDrgCodes(string text)
        {
            if (!string.IsNullOrEmpty(text))
            {
                var list = _drgservice.GetFilteredDRGCodes(text, Helpers.DefaultDrgTableNumber);
                var filteredList = list.Select(item => new
                {
                    ID = item.DRGCodesId,
        ...
        {
            var drg = _drgservice.GetDrgCodesById(drgCodeId);
            return drg;
        }
        ...
        public ActionResult GetDiagnosisByCodeId(string Id, int Pid, int Eid)
        {
            ...
            var diagnosisCode = _dcservice.GetDiagnosisCodeByCodeId(Id, Helpers.DefaultDiagnosisTableNumber);
            var drgCode = _drgservice.GetDrgCodesobjByCodeValue(Id, Helpers.DefaultDrgTableNumber);
            var encounterDiagnosis = _service.GetDiagnosisList(Convert.ToInt32(Pid),
                Convert.ToInt32(Eid));
            if (encounterDiagnosis.Any(x => x.DiagnosisType == Convert.ToInt32(DiagnosisType.Primary)))
            {
                Dtype =
        ...
        {
            var isExists = _service.CheckIfDuplicateDiagnosisAgainstCurrentEncounter(diagnosisCode, encounterId, diagnosisId);
            return isExists;
        }
        ...
        public bool SaveDiagnosisOrderActivity(Diagnosis model)
        {
            try
            {
                var drgCode = _drgservice.GetDrgCodeById(Convert.ToInt32(model.DRGCodeID), Helpers.DefaultDrgTableNumber);
                ...
                var alreadyExist = _oaService.GetOrderActivitiesByEncounterId(Convert.ToInt32(model.EncounterID), Helpers.DefaultCptTableNumber, Helpers.DefaultDrgTableNumber, Helpers.DefaultHcPcsTableNumber, Helpers.DefaultDrugTableNumber, Helpers.DefaultServiceCodeTableNumber, Helpers.DefaultDiagnosisTableNumber).FirstOrDefault(x => x.OrderType == 9);
                ...
                _oaService.AddUptdateOrderActivity(orderactivitySave);
                _oaService.ApplyOrderActivityToBill(Convert.ToInt32(model.CorporateID), Convert.ToInt32(model.FacilityID), Convert.ToInt32(model.EncounterID), string.Empty, 0);
        ...
        public JsonResult SaveDiagnosisCustomCode(DiagnosisCustomModel model)
        {
            ...
            var primaryDone = _service.CheckIfPrimaryDiagnosis(Convert.ToInt32(model.PatientID),
            ...
            {
                model.DRGCodeID = null;
                bool isExists = _service.CheckIfDuplicateDiagnosisAgainstCurrentEncounter(model.DiagnosisCode,
                    Convert.ToInt32(model.EncounterID), model.DiagnosisID);
                if (!isExists)
                {
                    result = _service.SaveDiagnosis(model, Helpers.DefaultDiagnosisTableNumber);
                    primaryDone = true;
                }
                else
            ...
            {
                majorDrgDone = _service.CheckIfPrimaryDiagnosis(Convert.ToInt32(model.PatientID),
            ...
            {
                majorCptDone = _service.CheckIfPrimaryDiagnosis(Convert.ToInt32(model.PatientID),
                      Convert.ToInt32(model.EncounterID), (int)DiagnosisType.CPT);
            }
            var currentDiagnosis = _service.GetCurrentDiagnosisData(Convert.ToInt32(model.PatientID),
        ...
        public JsonResult GetDiagnosisCodesData(List<string> diagCode)
        {
            ...
            if (diagCode.Count > 0)
            {
                list = _service.GetFilteredDiagnosis(diagCode, Helpers.DefaultDiagnosisTableNumber);
                var filteredList = list.Select(item => new
                {
                    ID = item.DiagnosisCode1,
                    Menu_Title = string.Format("{0} - {1}", item.DiagnosisFullDescription, item.DiagnosisCode1),
        ...
        {
            var diagnosislist = _service.GetDiagnosisList(Convert.ToInt32(Pid), Convert.ToInt32(Eid));
        ...
        {
            var list = _favService.GetFavoriteDiagnosisData(Helpers.GetLoggedInUserId(), Helpers.DefaultDiagnosisTableNumber, Helpers.DefaultDrgTableNumber);
        ...
        {
            var result = _service.GetCurrentDiagnosisData(patientId, encounterId);
