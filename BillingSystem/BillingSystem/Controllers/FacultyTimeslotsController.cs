...
namespace BillingSystem.Controllers
{
    ...
    /// <summary>
    ///     The faculty timeslots controller.
    /// </summary>
    public class FacultyTimeslotsController : BaseController
    {
        private readonly IPatientInfoService _piService;
        private readonly IFacilityStructureService _fService;
        private readonly IAppointmentTypesService _atService;
        private readonly ICountryService _cService;

        public FacultyTimeslotsController(IPatientInfoService piService, IFacilityStructureService fService, IAppointmentTypesService atService, ICountryService cService)
        {
        ...
        {
            var facultyTimeslotsView = new FacultyTimeslotsView
            {
                CurrentFacultyTimesSlots = new FacultyTimeslots(),
                FacultyTimesSlotsList =
                                                   new List<FacultyTimeslotsCustomModel>()
            };
            return View(facultyTimeslotsView);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult Index2(long? v)
        {
            return View();
        }
        ...
        public ActionResult GetAvailableTimeSlots(int facilityid, int physicianId, DateTime dateselected, string typeofproc)
        {
            ...
            var dt = DateTime.Now;
            var avialableTimeslotListing = schedulingBal.GetAvailableTimeSlots(
                facilityid,
                physicianId,
                dateselected,
                typeofproc, out dt);
        ...
        public ActionResult GetAvailableTimeSlotsUpdated(int facilityid, int physicianId, DateTime dateselected, string typeofproc)
        {
            var schedulingBal = new SchedulingBal();
            var dt = DateTime.Now;
            var avialableTimeslotListing = schedulingBal.GetAvailableTimeSlots(facilityid, physicianId, dateselected, typeofproc, out dt);
            var deptOpeningDays = schedulingBal.GetDeptOpeningDaysForPhysician(physicianId);
            var objToReturn =
                new { avialableTimeslotListing, deptOpeningDays };
        ...
        public JsonResult GetAllSchedulingData(string category, int facilityId, string corporateId, string selectedDate, string viewType)
        {
            ...
            using (var gcBal = new GlobalCodeBal())
            {
                ...
                if (gcList.Count > 0)
                {
                    ...
                    using (var pBal = new PhysicianBal())
                    {
                        ...
                        viewpath = string.Format("../FacultyTimeslots/{0}", PartialViews.PhysicianCheckBoxList);
                ...
                viewpath = string.Format("../FacultyTimeslots/{0}", PartialViews.StatusCheckBoxList);
            ...
            using (var facBal = new FacilityBal())
            {
                ...
                viewpath = string.Format("../FacultyTimeslots/{0}", PartialViews.LocationListView);
            ...
            var facilityDepartmentList = _fService.GetFacilityDepartments(cId, Convert.ToString(facilityId));
            ...
            viewpath = string.Format("../FacultyTimeslots/{0}", PartialViews.FacilityDepartmentListView);
            fDepView = RenderPartialViewToStringBase(viewpath, facilityDepartmentList);


            // Get the Facility Rooms list and bind to div #divFacilityRoomList
            var fRooms = _fService.GetFacilityRoomsCustomModel(cId, Convert.ToString(facilityId));
            viewpath = string.Format("../FacultyTimeslots/{0}", PartialViews.FacilityRoomsListView);
        ...
        public JsonResult OnPhysicianSelectionFromPreviousEncountersList(string category, int facilityId, int facilityStructureId)
        {
            ...
            appTypes = _fService.GetDepartmentAppointmentTypes(facilityStructureId, Convert.ToString(facilityId));
        ...
        public JsonResult OnChangeFacilityDropdownInAppointmentsOverview(int fId)
        {
            ...
            var facilityDepartments = _fService.GetFacilityDepartments(cId, Convert.ToString(fId));
        ...
        /// <param name="model">The model.</param>
        /// <returns></returns>
        //public JsonResult SavePatientSchedulingUpdated(List<SchedulingCustomModel> model)
        //{
        ...
        //            // check to add the physician department
        //            //var phId = !string.IsNullOrEmpty(model[0].PhysicianId)
        //            //    ? Convert.ToInt32(model[0].PhysicianId)
        //            //    : Convert.ToInt32(model[0].AssociatedId);

        //            //var physicianBal = new PhysicianBal();
        //            //var phys = physicianBal.GetPhysicianById(phId);
        //            //var physicianDeptid = phys != null ? phys.FacultyDepartment : string.Empty;


        //            //-------------------########### SAVING PATIENT DETAILS SECTION starts here  ###########-----------------------------

        //            //var patientBal = new PatientInfoBal();
        //            //var patientinfo = model[0].AssociatedId != 0
        //            //    ? patientBal.GetPatientInfoById(model[0].AssociatedId)
        //            //    : new PatientInfo
        //            //    {
        ...
        //            //        PersonLastName =
        //            //            model[0].PatientName.Replace(model[0].PatientName.Split(' ')[0], string.Empty)
        //            //                .TrimStart(),
        //            //        IsDeleted = false,
        //            //    };
        ...
        //            //pId = patientBal.AddUpdatePatientInfo(patientinfo);


        //            //// Save Phone and Email Details of Patient.
        //            //if (pId > 0)
        //            //{
        //            //    var phoneBal = new PatientPhoneBal();
        //            //    phoneBal.SavePatientPhoneInfo(pId, (int)(PhoneType.MobilePhone),
        //            //        model[0].PatientPhoneNumber, loggedInUserId, currentDateTime);

        //            //    var pLoginBal = new PatientLoginDetailBal();
        //            //    pLoginBal.UpdatePatientEmailId(pId, model[0].PatientEmailId);
        //            //}
        ...
        //            //DataTable dtScheduling = Helpers.ToDataTable(model);
        //            //oSchedulingBal.ValidateScheduling(dtScheduling);
        ...
        //                var item = model.FirstOrDefault(a => a.ScheduleFrom == model[0].ScheduleFrom);
        ...
        //                isAllReadyAppointed = isRoomalloted != null && isRoomalloted.IsAppointed;
        ...
        public JsonResult SaveAppointment(List<SchedulingCustomModel> model)
        {
            var isAllReadyAppointed = false;
            var notAvialableTimeslotslist = new List<NotAvialableTimeSlots>();
            var roomEquipmentnotaviable = new List<NotAvialableRoomEquipment>();
            ...
            if (model != null)
            {
                ...
                using (var oSchedulingBal = new SchedulingBal())
                {
                    ...
                    //-------------------########### SAVING PATIENT DETAILS SECTION ends here ###########-----------------------------
                    //Mapper.Initialize(cfg => cfg.CreateMap<SchedulingCustomModel, Scheduling>());
                    var mainList = model.Select(Mapper.Map<SchedulingCustomModel, Scheduling>).ToList();
                    var dtScheduling = Helpers.ToDataTable(mainList);
                    var isValid = 0;
                    var recurringList = oSchedulingBal.ValidateScheduling(dtScheduling, facilityId, loggedInUserId, out isValid);

                    var ss1 = recurringList.Where(s => s.IsRecurring != true).GroupBy(g => g.ScheduleFrom).Where(f => f.Count() > 1).Select(y => y.Key).ToList();
        ...
        public ActionResult GetFilteredSchedulerData(List<SchedularTypeCustomModel> filters)
        {
            ...
            {
                rList = _fService.GetFacilityRoomsByDepartments(Helpers.GetSysAdminCorporateID(), facilityid, dep, roomsList);
            }
        ...
        public JsonResult GetAllDataOnPhysicianSelection(string category, int facilityId, int deptId)
        {
            ...
            //######################--"Get Facility Departments and rooms" Code starts Here--#####################
            // Get the facility Department list and bind to div #divFacilityDepartmentList
            facilityDepartmentList = _fService.GetFacilityDepartments(cId, Convert.ToString(facilityId));
            ...
            appTypes = _fService.GetDepartmentAppointmentTypes(deptId, facilityId, cId, true);
            //######################--"Get Facility Departments and rooms" Code Ends Here--#####################
            ...
            if (model != null)
            {
                ...
                using (var oSchedulingBal = new SchedulingBal())
                {
                    ...
                    foreach (var item in model.Where(item => !string.IsNullOrEmpty(item.RemovedAppointmentTypes)))
                        oSchedulingBal.DeleteHolidayPlannerData(string.Empty,
                            Convert.ToInt32(item.RemovedAppointmentTypes), Convert.ToInt32(item.SchedulingType), "");
                    ...
                    Helpers.SendAppointmentNotification(model, model[0].PatientEmailId,
                        Convert.ToString(model[0].EmailTemplateId), Convert.ToInt32(model[0].PatientId),
                        Convert.ToInt32(model[0].PhysicianId), status);
                }
            }
