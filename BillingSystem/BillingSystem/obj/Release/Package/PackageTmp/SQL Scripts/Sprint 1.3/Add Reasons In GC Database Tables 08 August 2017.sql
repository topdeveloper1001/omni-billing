...
IF NOT Exists (Select 1 From GlobalCodeCategory Where GlobalCodeCategoryValue = '80441')
Begin
	...
	Select TOP 1 0 As [FacilityNumber],'80441' As [GlobalCodeCategoryValue]
	,'Reason for Clinician Off Times' As [GlobalCodeCategoryName],1 As [CreatedBy]
	...
	From GlobalCodeCategory Where GlobalCodeCategoryValue = '80440'
End


IF NOT Exists (Select 1 From GlobalCodes Where GlobalCodeCategoryValue = '80441')
Begin
	Declare @TempValues Table (N nvarchar(100),Id int)

	INSERT INTO @TempValues
	Select N,ROW_NUMBER() OVER(ORDER BY N) As Id
	From
	(
		Select 'Meeting' As N
		UNION 
		Select 'Vacation' As N
		UNION
		Select 'Not Scheduled' As N
		UNION 
		Select 'Admin Time' As N
		UNION 
		Select 'Out-Office' As N
		UNION
		Select 'General' As N
	) a
	...
	Select '0' As [FacilityNumber],'80441' As [GlobalCodeCategoryValue]
      ,Id as [GlobalCodeValue],N as [GlobalCodeName],N as [Description],'' As [ExternalValue1]
	  ,'' AS [ExternalValue2],'' AS [ExternalValue3]
      ,'' AS [ExternalValue4],'' AS [ExternalValue5],Id AS [SortOrder],1 As [IsActive],1 As [CreatedBy]
