IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SPROC_ValidateSchedulerAppointment')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE SPROC_ValidateSchedulerAppointment
		 
		 GO

/****** Object:  StoredProcedure [dbo].[SPROC_ValidateSchedulerAppointment]    Script Date: 3/22/2018 8:24:59 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SPROC_ValidateSchedulerAppointment]
(
...
BEGIN
	...
	Set @LocalDateTime = (Select dbo.GetCurrentDatetimeByEntity(@pFId))
	...
	Declare @SchedulingTableTemp Table(SchedulingId bigint,[AssociatedId] [int] NULL,[AssociatedType] [nvarchar](50) NULL,[SchedulingType] [nvarchar](50) NULL,[Status] [nvarchar](50) NULL,[StatusType] [nvarchar](50) NULL,
	...
	--- Holidays Temp Table 
	Declare @HolidaysTableTemp Table([SchedulingId] INT,[ScheduleFrom] [datetime] NULL,[ScheduleTo] [datetime] NULL)
	...
	Set @PhyDeptId = (select TOP 1 FacultyDepartment from Physician where ID = @CUR_PhysicianId)
	...
	Begin
		...
		SET @RecuranceType = (Select TOP 1 [Value] from @SplitTable where ID = 1)
		...
		BEGIN
			...
			WHile ((SELECT Count(1) FROM @pSchedulingList WHERE [EventId] = @DynamicEventId) > 0 OR (SELECT Count(1) FROM @SchedulingTableTemp WHERE [EventId] = @DynamicEventId) > 0)
			BEGIN
			...
			If (DATEADD(ss,@CUR_RecEventlength,@pSelectedDate) >@LocalDateTime)
				...
				BEGIN
					...
					IF NOT  EXISTS (SELECT  1 from @pSchedulingList where [PhysicianId] = @CUR_PhysicianId and ScheduleFrom = @pSelectedDate and ScheduleTo = DATEADD(ss,@CUR_RecEventlength,@pSelectedDate))
					BEGIN			
						Insert INTO @SchedulingTableTemp
						Select @CUR_SchedulingId, @CUR_AssociatedId,@CUR_AssociatedType,@CUR_SchedulingType,@CUR_Status,@CUR_StatusType,@pSelectedDate,DATEADD(ss,@CUR_RecEventlength,@pSelectedDate),@CUR_TypeOfVisit
							...
							BEGIN
								...
								BEGIN
									...
									BEGIN
										...
										BEGIN
											IF NOT EXISTS (Select 1 from @pSchedulingList where [PhysicianId] = @CUR_PhysicianId and ScheduleFrom = @WeekStartDate and ScheduleTo = DATEADD(ss,@CUR_RecEventlength,@WeekStartDate))
											BEGIN
												Insert INTO @SchedulingTableTemp
												Select @CUR_SchedulingId,@CUR_AssociatedId,@CUR_AssociatedType,@CUR_SchedulingType,@CUR_Status,@CUR_StatusType,@WeekStartDate,DATEADD(ss,@CUR_RecEventlength,@WeekStartDate),@CUR_TypeOfVisit
												,@CUR_PhysicianSpeciality,@CUR_PhysicianId,@CUR_TypeOfProcedure,@CUR_FacilityId,@CUR_CorporateId,@CUR_Comments,@CUR_Location,@CUR_CreatedBy,@CUR_CreatedDate,@CUR_ModifiedBy,@CUR_ModifiedDate,@CUR_IsActive,@CUR_DeletedBy,@CUR_DeletedDate
												,@CUR_ExtValue1,@CUR_ExtValue2,@CUR_ExtValue3,@CUR_ExtValue4,@CUR_ExtValue5
												,@CUR_IsRecurring,@CUR_RecType,@CUR_RecPattern,@CUR_RecEventlength,@CUR_RecEventPId,@CUR_RecurringDateFrom,@CUR_RecurringDateTill,@DynamicEventId,@CUR_WeekDay,@CUR_EventParentId
												,@CUR_RoomAssigned,@CUR_EquipmentAssigned,@PhysicianReferredBy
	...
	SELECT SchedulingId,ScheduleFrom,ScheduleTo from dbo.Scheduling where AssociatedId = @PhyidH and AssociatedType in ('2')
