IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SPROC_GetSelectedBedInformation')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE SPROC_GetSelectedBedInformation
GO

/****** Object:  StoredProcedure [dbo].[SPROC_GetSelectedBedInformation]    Script Date: 22-03-2018 19:42:26 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SPROC_GetSelectedBedInformation] 
(  
@pPatientID int,
@pBedID int,
@pOverRideServiceCode nvarchar(20)
)
AS  
BEGIN
...
Select @FacilityStructureID = FacilityStructureID,@AssignedBedType= BedType from UBedMaster Where BedID = @pBedID;

If @pOverRideServiceCode <> '0' 
Begin
	Select @OverRidenBedType =BedTypes from BedRateCard Where ServiceCodeValue = @pOverRideServiceCode;
	Set @AssignedSince = @LocalDateTime --@LocalDateTime
End
...
--Set @RetOutput = 'Bed Assigned:-' +isnull(@BedNumber,' ')+ ' Dated:-'+Cast(isnull(@AssignedSince,@LocalDateTime) as nvarchar(20)) +' In Room:-'+isnull(@InRoom,' ')+' In Department:-'+Isnull(@InDepartment,' ')+' on Floor:-'+isnull(@OnFloor,' ')
