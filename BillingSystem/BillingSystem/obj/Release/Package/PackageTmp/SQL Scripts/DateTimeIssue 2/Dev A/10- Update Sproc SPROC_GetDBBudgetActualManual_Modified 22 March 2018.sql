IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SPROC_GetDBBudgetActualManual_Modified')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE SPROC_GetDBBudgetActualManual_Modified

/****** Object:  StoredProcedure [dbo].[SPROC_GetDBBudgetActualManual_Modified]    Script Date: 3/22/2018 7:20:46 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SPROC_GetDBBudgetActualManual_Modified]
(  
...
IF((Select Count(FID) from @SelT) > 1)
	Begin
		...
		Select 1,DID.IndicatorNumber,DID.SubCategory1,DID.SubCategory2,SUM(CAST(DID.StatisticData as DECIMAL)), DID.[Month],DID.[Year],9999,@CID,1,@CurrentDate,DID.ExternalValue1 
