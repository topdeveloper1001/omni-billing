IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'GenerateClaimFile')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE GenerateClaimFile

/****** Object:  StoredProcedure [dbo].[GenerateClaimFile]    Script Date: 3/22/2018 7:41:22 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GenerateClaimFile]
	(
...
Declare @CurrentDate datetime= (Select dbo.GetCurrentDatetimeByEntity(0))
	...
	Declare @RecordCount int,@ID bigint=0,@FetchEID bigint,@AID bigint,@HeaderXMLDesc nvarchar(500),@XH xml,@XC xml,@XE xml,@XA xml,@XO xml,
	...
	Declare @PatientId int= 0,@EncType nvarchar(20)='2';
...
Begin
	 -- Handling next number for XFileHeader - STARTS
		...
		VALUES ('REQ',@FSenderID,@ReceiverID,@CurrentDate,@RecordCount,@DispositionFlag,'','1',@CurrentDate,'',NULL,'UspGenerateClaimFile',@CurrentDate,@CorporateID,@SenderID)
...
--- Prepare XML Header part - STARTS
	...
	*/
--Set @XH = (Select @FacilityNumber as SenderID,@PayerID as ReceiverID,@CurrentDate as TransactionDate,@RecordCount as RecordCount,@DispositionFlag as DispositionFlag for XML PATH ('Header'))
Set @XH = (Select @FSenderID as SenderID,@ReceiverID as ReceiverID,@CurrentDate as TransactionDate,@RecordCount as RecordCount,@DispositionFlag as DispositionFlag for XML PATH ('Header'))
...
-- Prepare XML under Claim parts - ENDS
	...
	UpDate BillHeader Set ClaimID = @FetchEID, FileID = @ID	Where BillHeaderID = @FetchEID;
...
--Insert into [dbo].[XFileHeader] values(@ID,'REQ',@SenderID,@PayerID,@CurrentDate,@RecordCount,@DispositionFlag,'','1',@CurrentDate,'',NULL,'UspGenerateClaimFile',@CurrentDate,@CorporateID,@SenderID)

--- Saving XML file in DB for Backup purpose if Network fails and to Check any issues on front end
Insert into [dbo].[XFileXML] values(@ID,@XMLFile,@CurrentDate)
