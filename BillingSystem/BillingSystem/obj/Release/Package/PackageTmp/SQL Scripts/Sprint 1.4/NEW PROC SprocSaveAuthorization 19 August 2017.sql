IF EXISTS (SELECT * 
           ...
           WHERE  specific_schema = N'dbo' 
                  AND specific_name = N'SprocSaveAuthorization') 
...
-- =============================================
CREATE PROCEDURE SprocSaveAuthorization
(
	@Id int,
	@pCId bigint,
	@pFId bigint,
...
BEGIN
	...
	,@Status int=@Id,@TimeZone nvarchar(100)
	...
	IF @pCreatedDate IS NULL AND ISNULL(@TimeZone,'') !=''
		Set @pCreatedDate = dbo.GetLocalTimeBasedOnTimeZone(@TimeZone,GETDATE())
	...
	If @Id = 0 And Not Exists (
		...
		And CAST(AuthorizationEnd as date) = CAST(@pEnd as date))
	Begin
