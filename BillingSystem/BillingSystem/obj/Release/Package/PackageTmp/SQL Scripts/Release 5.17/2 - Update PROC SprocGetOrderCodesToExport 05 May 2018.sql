IF OBJECT_ID('SprocGetOrderCodesToExport','P') IS NOT NULL
	DROP PROCEDURE SprocGetOrderCodesToExport

GO

/****** Object:  StoredProcedure [dbo].[SprocGetOrderCodesToExport]    Script Date: 6/5/2018 7:13:48 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	END
	ELSE IF @pCodeType = 'BM'			--'0'
	BEGIN
		Insert into @GeneralCodesTable(Id,OtherValue1,OtherValue2,Code,CodeDescription,EffectiveFrom,EffectiveTill)
		Select Id,[Name],[Type],Code,[Description],EffectiveStartDate,EffectiveEndDate
		From BillingModifier where (@pSearchText='' OR (Code like '%'+ @pSearchText  +'%' 
		or [Description] like '%'+ @pSearchText  +'%'))
		AND (CAST(@LocalDateTime as date) 
		between CAST(EffectiveStartDate as date) And ISNULL(EffectiveEndDate,@LocalDateTime))
		AND ISNULL(IsActive,1) = 1 and FacilityId=@pFId

		SET @columns = 'Code,Name,Description,Type,EffectiveStartDate,EffectiveEndDate'
	END
	ELSE IF @pCodeType = 'POS'			--'0'
	BEGIN
		Insert into @GeneralCodesTable(Id,OtherValue1,Code,CodeDescription,EffectiveFrom,EffectiveTill)
		Select Id,[Name],Code,[Description],EffectiveStartDate,EffectiveEndDate
		From PlaceOfService where (@pSearchText='' OR (Code like '%'+ @pSearchText  +'%' 
		or [Description] like '%'+ @pSearchText  +'%'))
		AND (CAST(@LocalDateTime as date)
		between CAST(EffectiveStartDate as date) And ISNULL(EffectiveEndDate,@LocalDateTime))
		AND ISNULL(IsActive,1) = 1 and FacilityId=@pFId

		SET @columns = 'Code,Name,Description,EffectiveStartDate,EffectiveEndDate'
	END
