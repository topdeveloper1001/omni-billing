...
BEGIN
	Begin Try
		...
		*/
		...
		Declare @TRoom Table (RoomId bigint,EqId bigint,DepId bigint,IsAppointed bit,SchId bigint,IsEquipmentRequired bit)
		...
		Begin
			...
			*/
			INSERT INTO @TRoom (RoomId,EqId,DepId,IsAppointed,IsEquipmentRequired)
			Exec SprocCheckRoomsAndOthersAvailibilty @FId,@pAppointmentTypeId
			...
			BEGIN
				...
				,[IsRecurring],[EventId],[WeekDay],[EventParentId],[RoomAssigned],[EquipmentAssigned],[PhysicianReferredBy],ExtValue5)
				Select @pPatientId,'1','1' As SchedulingType,'8',NULL As StatusType,@ScheduleTimeFrom,@ScheduleTimeTill
				...
				, 0 As IsRecurring,@pEventId,@pWeekDay,@pEventParentId,RoomId,EqId,@pClinicianReferredBy,'ios'
				From @TRoom
			...
			BEGIN
				...
				Where S.SchedulingId=@pId And S.[Status] IN ('1','8') --Check Appointment Status: It should be Always be Initial Booking in case of updates.
