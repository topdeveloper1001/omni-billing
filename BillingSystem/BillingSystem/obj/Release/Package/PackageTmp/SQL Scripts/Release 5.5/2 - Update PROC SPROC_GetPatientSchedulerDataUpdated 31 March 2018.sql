IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SPROC_GetPatientSchedulerDataUpdated')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
	DROP PROCEDURE [dbo].[SPROC_GetPatientSchedulerDataUpdated]
GO

/****** Object:  StoredProcedure [dbo].[SPROC_GetPatientSchedulerDataUpdated]    Script Date: 3/31/2018 11:14:52 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
Create PROCEDURE [dbo].[SPROC_GetPatientSchedulerDataUpdated]
(
@pVT int=3,
...
--cross apply TX.X.nodes('*') as TN(N)
	...
	Begin
		INSERT INTO @TableToReturn
		Exec [SPROC_GetSchedulingEvents_Back] @pVT,@pDate, @pPhyIds, @pFId
	End

	--PRINT '@TableToReturn----1'
	...
	--PRINT '@TableToReturn-----1werewiewfniew2'
	--Select * From @TableToReturn2
	...
	,PhysicianId nvarchar(100),ExtValue5 nvarchar(100))
	...
	,(Select TOP 1 G.GlobalCodeName From GlobalCodes G Where G.GlobalCodeCategoryValue = '4904' And G.GlobalCodeValue = Ap.DefaultTime) As TimeSlotTimeInterval
	...
	Exec SPROC_GetPatientNextSchedulingEvents @pDate, @pPatientId
END
