...
CREATE PROCEDURE SprocGetOrderTypeCategoriesByFacility
(
	@pUserId bigint,
	@pFacilityId bigint,
	@pStatusId bit=1
...
Begin
	...
	,ExternalValue4=
	(
		   STUFF((SELECT distinct ', ' + cast(G2.GlobalCodeCategoryName as varchar(10))
           FROM dbo.Split(',',G.ExternalValue4) G1
		   INNER JOIN GlobalCodeCategory G2 ON G2.GlobalCodeCategoryValue=G1.IDValue AND G2.FacilityNumber='0' AND G2.ExternalValue3='OrderCategory'
           FOR XML PATH('')),1,1,'')
	)
