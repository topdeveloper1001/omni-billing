...
function dhtmlXTreeObject(htmlObject, width, height, rootId) {
    ...
    this.lineArray = new Array("line2.gif", "line3.gif", "line4.gif", "", "", "line1.gif");
    this.minusArray = new Array("minus2.gif", "minus3.gif", "minus4.gif", "minus.gif", "minus5.gif");
    this.plusArray = new Array("plus2.gif", "plus3.gif", "plus4.gif", "plus.gif", "plus5.gif");
    this.imageArray = new Array("", "folderOpen.gif", "folderClosed.gif");
...
function dhtmlXTreeItemObject(itemId, itemText, parentObject, treeObject, actionHandler, mode, acolor, scolor,balanceHours, contractTaskData) {
    this.htmlNode = "";
    this.acolor = acolor;
    this.scolor = scolor;
    ...
    if (contractTaskData != undefined) {
        this.customerName = contractTaskData.customerName;
        this.productName = contractTaskData.productName;
        this.contractUsage = contractTaskData.contractUsage;
        this.balanceTimes = contractTaskData.balanceTimes;
        this.projectId = contractTaskData.projectId;
        this.stepSize = contractTaskData.stepSize;
        this.selectedTab = contractTaskData.selectedTab;
        this.unit = contractTaskData.unit;
        this.Comment = contractTaskData.comment;
    }
...
*     @topic: 2
*/
dhtmlXTreeObject.prototype._attachChildNode = function (parentObject, itemId, itemText, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode, afterNode, acolor, scolor, balanceHours, contractTaskData) {
    ...
    if (!(n = this._idpull[itemId]) || n.span != -1) {
        ...
        n = Nodes[Count] = new dhtmlXTreeItemObject(itemId, itemText, parentObject, this, itemActionHandler, 1, acolor, scolor, balanceHours, contractTaskData);
        //n.htmlNode.bgColor = n.acolor;//set task background

        n.htmlNode.style.background = n.acolor;//set task background
        n.htmlNode.style.color = n.scolor;
...
dhtmlXTreeObject.prototype._parseItem = function (c, temp, preNode, befNode) {
    ...
    var a = c.get_all();
    var contractTaskData = { 'customerName': a.customerName, 'productName': a.productName, 'contractUsage': a.contractUsage, 'projectId': a.projectId, 'stepSize': a.stepSize, 'selectedTab': a.selectedTab,'comment':a.comment, 'unit': a.unit, 'balanceTimes': a.balanceTimes };
    ...
    if (this.waitUpdateXML) {
        ...
        var newNode = this._attachChildNode(temp, a.id, a.text, 0, a.im0, a.im1, a.im2, zST.join(","), a.child, (befNode || 0), preNode, a.acolor, a.scolor, a.balanceHours, contractTaskData);
    if (a.tooltip)
...
dhtmlXTreeObject.prototype._unselectItems = function () {
    for (var i = 0; i < this._selected.length; i++) {
        ...
        node.span.className = "standartTreeRow";
        if (node.acolor) node.span.style.color = node.scolor;//node.acolor;
        node.i_sel = false;
    }
    this._selected = new Array();
}
...
dhtmlXTreeObject.prototype.onDeleteTreeItemClick = function (e) {
    DeleteTreeItem(e.currentTarget.id);

}
dhtmlXTreeObject.prototype.onGetContractUsage = function (e) {
    GetContractUsage(e.currentTarget.id);
...
dhtmlXTreeObject.prototype._createItem = function (acheck, itemObject, mode) {
    var visibleStatus = "none";
    var status = "block";
    if (itemObject.selectedTab != undefined && itemObject.selectedTab == "contract") {
        visibleStatus = "block";
    }
    if (itemObject.selectedTab != undefined && itemObject.selectedTab == "absenceContract" || itemObject.selectedTab == 'internalTask') {
        status = "none";
    }
    ...
    i.id = itemObject.id;
    i.balanceHour = itemObject.balanceHours;
    i.className = "fa fa-times icon-pos pull-right";
    i.style.display = status;
    i.onclick = this.onDeleteTreeItemClick;
    ...
    var divCustomerUnit = document.createElement('div');
    divCustomerUnit.style.styleFloat = "left";
    divCustomerUnit.style.lineHeight = "15px";
    if (itemObject.selectedTab != "absenceContract") {
        divCustomerUnit.innerHTML = "<i class='fa fa-clock-o'>";
        if (itemObject.unit == "Times") {
            divCustomerUnit.innerHTML = "<i class=' fa fa-align-justify fa-rotate-90'>";
        }
    }


    divCustomerName.className = " CustomerName";
    if (itemObject.selectedTab == "absenceContract") {
        divCustomerName.style.color = "#666666";
    }
    
    if (itemObject.selectedTab == "internalTask") {
        divCustomerName.style.display = "none";
        
    }

    divCustomerName.innerHTML = itemObject.customerName;
    divCustomerName.title = itemObject.customerName;

    var divBalanceTimes = document.createElement('div');
    divBalanceTimes.className += " BalanceTimes";
    divBalanceTimes.style.display = "none";


    var divBalanceContractHours = document.createElement('div');
    divBalanceContractHours.innerHTML = itemObject.balanceHours;
    //div2.style.styleFloat = "right";
    //div2.style = "float:right;";
    var selctedTabClass = 'contractClass';
    if (itemObject.selectedTab == 'priorityContract') {
        selctedTabClass = 'priorityClass';
    }
    if (itemObject.selectedTab == 'absenceContract') {
        selctedTabClass = 'absenceClass';
    }
    if (itemObject.selectedTab == 'internalTask') {
        selctedTabClass = 'internalTaskClass';
    }
    divBalanceContractHours.className += " BalanceContractHours " + selctedTabClass;
    if (itemObject.unit == "Hours" || itemObject.selectedTab == 'absenceContract' || itemObject.selectedTab == 'internalTask') {
        divBalanceContractHours.innerHTML = itemObject.balanceHours;
        divBalanceContractHours.style.display = status;
    }
    else {
        divBalanceContractHours.style.display = "none";
        divBalanceTimes.innerHTML = itemObject.balanceTimes;
        divBalanceTimes.className += " BalanceTimes " + selctedTabClass;
        divBalanceTimes.style.display = "block";

    }

    var divProductName = document.createElement('div');
    //if (itemObject.selectedTab == 'contract') {

    //div3.style = "float:left; clear:both; overflow:hidden; text-overflow: ellipsis; white-space: nowrap; width: 58%;";
   
    //}
    
    //div3.style.styleFloat = "left";
    
    // Add diff. class to show just name in case of internal task
    if (itemObject.selectedTab == 'internalTask') {
        divProductName.className = " InternalTaskName ";
    } else {
        divProductName.className = " ProductName";
        divProductName.style.clear = "both";
    }

    divProductName.innerHTML = itemObject.productName;
    divProductName.title = itemObject.productName;
    divProductName.style.display = status;
    if (itemObject.selectedTab == 'internalTask') {
        divCustomerUnit.style.display = "none";  // Hide unit icon in case of internal tasks
        divProductName.style.display = 'block';
        divCustomerName.style.display = 'none';
        divProductName.style.color = "#666666";
    }

    var divContractUsage = document.createElement('div');
    //div4.style = "float:right;";
    //div4.style.styleFloat = "right";
    
    divContractUsage.className = " ContractUsage";
    divContractUsage.innerHTML = itemObject.contractUsage;
    //Himanshu 1Sep2015: Jira 687
    //if contract has comment then bind comment icon and show comment text in tooltip
    if (itemObject.Comment) {
        divContractUsage.title = itemObject.Comment;
        divContractUsage.innerHTML = '<i class="fa fa-comment"></i> ' + itemObject.contractUsage;
    }
    divContractUsage.id = itemObject.id;
    divContractUsage.style.display = visibleStatus;
    divContractUsage.onclick = this.onGetContractUsage;

    var divContractName = document.createElement('div');
    divContractName.className = "ContractName";
    divContractName.style.display = "none";
    divContractName.innerHTML = itemObject.label;

    var divProjectId = document.createElement('div');
    divProjectId.style.display = "none";
    divProjectId.className += " ProjectId";
    divProjectId.innerHTML = itemObject.projectId;

    var divStepSize = document.createElement('div');
    divStepSize.style.display = "none";
    divStepSize.className += " StepSize";
    divStepSize.innerHTML = itemObject.stepSize;

    var divSelectedTab = document.createElement('div');
    divSelectedTab.style.display = "none";
    divSelectedTab.className += " SelectedTab";
    divSelectedTab.innerHTML = itemObject.selectedTab;
    ...
    //if (itemObject.Comment) {
    //    divComment.title = itemObject.Comment;
    //    divComment.innerHTML = '<i class="fa fa-comment"></i>';
    //}
    ...
    divUnit.innerHTML = itemObject.unit;

    itemObject.span = document.createElement('span');
    itemObject.span.className = "standartTreeRow";
    itemObject.span.setAttribute("TaskId", itemObject.id);
    if (this.mlitems) {
    ...
    if (_isIE && _isIE > 7) td2.style.width = "100%"//"999999px";
    else if (!window._KHTMLrv) td2.style.width = "100%";

    //      itemObject.span.appendChild(document.createTextNode(itemObject.label));
    //itemObject.span.innerHTML = div1 + div2; //itemObject.label + "<br/>(" + itemObject.scolor + ")";


    itemObject.span.appendChild(divCustomerUnit);
    itemObject.span.appendChild(divCustomerName);
    itemObject.span.appendChild(divBalanceTimes);
    itemObject.span.appendChild(divBalanceContractHours);
    itemObject.span.appendChild(divProductName);
    itemObject.span.appendChild(divContractUsage);
    itemObject.span.appendChild(divContractName);
    itemObject.span.appendChild(divProjectId);
    itemObject.span.appendChild(divStepSize);
    itemObject.span.appendChild(divSelectedTab);
   // itemObject.span.appendChild(divComment);
    itemObject.span.appendChild(divUnit);
    ...
    itemObject.span.style.padding = "0 0px 35px 0px "; td2.style.verticalAlign = "";
