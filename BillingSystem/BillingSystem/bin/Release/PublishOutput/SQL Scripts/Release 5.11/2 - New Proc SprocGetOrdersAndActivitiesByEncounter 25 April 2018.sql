...
/****** Object:  StoredProcedure [dbo].[SprocGetOrdersAndActivitiesByEncounter]    Script Date: 4/25/2018 1:48:24 PM ******/
SET ANSI_NULLS ON
GO
...
Create PROCEDURE SprocGetOrdersAndActivitiesByEncounter
(
@pEncounterId bigint
)
AS
BEGIN
	...
	,[Status]=(CASE WHEN CategoryId = @PharmacyCategoryid AND OrderStatus not in ('3','4','9') AND IsApproved = 0 
					...
					ELSE
							(Select TOP 1 GlobalCodeName From GlobalCodes 
									Where GlobalCodeCategoryValue = @pOrderStatusGCC And GlobalCodeValue = OrderStatus)
	...
	,SpecimenTypeStr=(Select TOP 1 GlobalCodeName from GlobalCodes 
			where GlobalCodeCategoryValue=@pLabTestResultSpecimenGCC 
	...
	(Select TOP 1 GlobalCodeCategoryName From GlobalCodeCategory Where GlobalCodeCategoryValue = Cast(OrderCategoryID As Varchar) 
			AND (@FId=0 OR FacilityNumber=CAST(@FId as nvarchar))) CategoryName,
	(Select TOP 1 GlobalCodeName From GlobalCodes Where GlobalCodeValue = CAST(OrderSubCategoryID as nvarchar) AND (@FId=0 OR FacilityNumber=CAST(@FId as nvarchar))) SubCategoryName
	,OrderDescription= (Case WHEN OrderType='3' OR OrderType='CPT' THEN 
									(Select TOP 1 CodeDescription from CPTCodes Where CodeNumbering = OrderCode)
								WHEN OrderType='4' OR OrderType='HCPCS' THEN
									(Select TOP 1 CodeDescription from HCPCSCodes Where CodeNumbering = OrderCode)
								...
								WHEN OrderType='9' OR OrderType='DRG' THEN 
									(Select TOP 1 CodeDescription from DRGCodes Where CodeNumbering = OrderCode)
								ELSE '' END)
	,(Select TOP 1 GlobalCodeName From GlobalCodes Where GlobalCodeValue = Cast(OrderActivityStatus As Varchar) And GlobalCodeCategoryValue = @OrderActivityStatusGCC) [Status],
	(Select TOP 1 GlobalCodeName From GlobalCodes Where GlobalCodeValue = Cast(OrderType As Varchar) And GlobalCodeCategoryValue = @pOrderTypeGCC 
	...
	,ResultUOMStr=(Case 
						...
						Then (Select GlobalCodeName From GlobalCodes 
									Where GlobalCodeValue = Cast(ResultUOM As Varchar) And GlobalCodeCategoryValue = @LabMeasurementValueGCC) 
						Else '' End)
	,'' As LabResultTypeStr,
