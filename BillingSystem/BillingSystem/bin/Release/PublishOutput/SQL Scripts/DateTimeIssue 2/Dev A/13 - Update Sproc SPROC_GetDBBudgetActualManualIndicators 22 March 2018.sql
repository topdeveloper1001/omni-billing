IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SPROC_GetDBBudgetActualManualIndicators')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE SPROC_GetDBBudgetActualManualIndicators

/****** Object:  StoredProcedure [dbo].[SPROC_GetDBBudgetActualManualIndicators]    Script Date: 3/22/2018 7:26:10 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SPROC_GetDBBudgetActualManualIndicators]
(  
	@CID int = 12,  --- CorporateID
	@FID int = 0,  --- FacilityID
	@TillDate date = '2015-01-04', --- Pass in the current date till which date you want data to be displayed
	@FacilityRelated nvarchar(40)=null, ---- FacilityType
	@FacilityRegion nvarchar(10) =null, ----- regionID
	@pIndicatorNumber nvarchar(20)='109'
...
Declare @Results table (CID int, FID int, IndicatorNumber nvarchar(50), DashBoard nvarchar(500), ResultType nvarchar(50),Result decimal(18,4), Tag nvarchar(10), FormatType int, EV2 nvarchar(50),Subcategory1 nvarchar(10),Subcategory2 nvarchar(10))

Declare @FormatResults table (CID int, FID int, INum nvarchar(50), DashBoard nvarchar(500), ResultType nvarchar(50), FormatType int, EV2 nvarchar(50),Subcategory1 nvarchar(10),Subcategory2 nvarchar(10),
...
IF((Select Count(FID) from @SelT) > 1)
	Begin
		...
		Select 1,DID.IndicatorNumber,DID.SubCategory1,DID.SubCategory2,SUM(CAST(DID.StatisticData as DECIMAL)), DID.[Month],DID.[Year],9999,@CID,1,GetDate(),DID.ExternalValue1 
			,DID.ExternalValue2,DID.ExternalValue3,1,Null 	from  [dbo].[DashboardIndicatorData] DID
			Where DID.CorporateId = @CID and FacilityId in (Select FID from @SelT) and DID.IndicatorNumber in (@pIndicatorNumber)
			GROUP BY DID.IndicatorNumber,DID.SubCategory1,DID.SubCategory2,DID.[Month],
...
Case When DID.ExternalValue1 = 1 Then 'CYTB' Else 'CYTA' END Tag,0,'',DID.SubCategory1,DID.SubCategory2 from [dbo].[DashboardIndicatorData] DID
...
and DID.IndicatorNumber in (@pIndicatorNumber)
--Group by DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,DID.ExternalValue1

---- Monthly
insert into @Results
Select DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,'',DID.ExternalValue1 ResultType, cast(DID.StatisticData as decimal(18,4)) Result,
Case When DID.ExternalValue1 = 1 Then 'CMB' Else 'CMA' END Tag,0,'',DID.SubCategory1,DID.SubCategory2 from [dbo].[DashboardIndicatorData] DID
-- inner join [dbo].[DashboardIndicators] DI on DI.IndicatorNumber = DID.IndicatorNumber and DI.CorporateId = @CID and DI.FacilityId  in (Select FID from @SelT)
Where DID.CorporateId = @CID and DID.FacilityId in (Select FID from @SelT) and DID.[Year] =@FiscalYear and DID.[Month] = @Month
and DID.IndicatorNumber in (@pIndicatorNumber)
-- Group by DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,DID.ExternalValue1
...
insert into @Results
Select DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,'',DID.ExternalValue1 ResultType, cast(DID.StatisticData as decimal(18,4)) Result,
Case When DID.ExternalValue1 = 1 Then 'PYTB' Else 'PYTA' END Tag,0,'',DID.SubCategory1,DID.SubCategory2 from [dbo].[DashboardIndicatorData] DID
-- inner join [dbo].[DashboardIndicators] DI on DI.IndicatorNumber = DID.IndicatorNumber and DI.CorporateId = @CID and DI.FacilityId  in (Select FID from @SelT)
Where DID.CorporateId = @CID and DID.FacilityId in (Select FID from @SelT) and DID.[Year] =@FiscalYear and DID.[Month] <= @Month
and DID.IndicatorNumber in (@pIndicatorNumber)
-- Group by DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,DID.ExternalValue1
...
insert into @Results
Select DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,'',DID.ExternalValue1 ResultType, cast(DID.StatisticData as decimal(18,4)) Result,
Case When DID.ExternalValue1 = 1 Then 'PMB' Else 'PMA' END Tag,0,'',DID.SubCategory1,DID.SubCategory2 from [dbo].[DashboardIndicatorData] DID
-- inner join [dbo].[DashboardIndicators] DI on DI.IndicatorNumber = DID.IndicatorNumber and DI.CorporateId = @CID and DI.FacilityId  in (Select FID from @SelT)
Where DID.CorporateId = @CID and DID.FacilityId in (Select FID from @SelT) and DID.[Year] =@FiscalYear and DID.[Month] = @Month
and DID.IndicatorNumber in (@pIndicatorNumber)
-- Group by DID.CorporateId, DID.FacilityId,DID.IndicatorNumber,DID.ExternalValue1
