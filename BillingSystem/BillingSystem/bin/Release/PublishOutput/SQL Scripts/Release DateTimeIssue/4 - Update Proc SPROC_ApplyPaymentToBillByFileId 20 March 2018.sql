IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SPROC_ApplyPaymentToBillByFileId')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE SPROC_ApplyPaymentToBillByFileId
GO

/****** Object:  StoredProcedure [dbo].[SPROC_ApplyPaymentToBillByFileId]    Script Date: 20-03-2018 15:56:24 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SPROC_ApplyPaymentToBillByFileId]
(  
@pCorporateID int,
@pFacilityID int,
@pFileId int
...
BEGIN
    ...
    --    BEGIN TRANSACTION
		    ...
		    DECLARE @Cur_PaymentID INT, @Cur_PayDate Datetime,@Cur_PayAmount decimal (18,2),@Cur_PayReference nvarchar(50), @Cur_PayFor int,@Cur_PayBy int,
					...
					@Cur_ARFileID bigint,@Cur_PayNETAmount decimal (18,2),@CUR_XActivityParsedId nvarchar(20)
			...
			PayBillID, PayActivityID,PayEncounterID,PayFacilityID,PayCorporateID,PayXADenialCode,PayXAFileHeaderID,PayNETAmount,XActivityParsedId
			...
			Order by PayBillID ;
				...
				@Cur_PayActivityID,@Cur_PayEncounterID,@Cur_PayFacilityID,@Cur_PayCorporateID,@Cur_PayXADenialCode,@Cur_ARFileID,@Cur_PayNETAmount,@CUR_XActivityParsedId
				...
				IF(@Cur_PayActivityID IS NULL)  --- 3rd party XML file
				BEGIN
					Set @Cur_PayActivityID = (Select BillActivityId from BillActivity where XActivityParsedId = @CUR_XActivityParsedId)
				END
					...
					Begin
						...
						Begin
							Update BillHeader Set PaymentAmount = isnull(@TotalAmountPaid,0), PaymentReference = @TotalPaymentReference, 
								DateSettlement = @TotalPaymentDate , DenialCode = @TotalDenialCode, ARFileID =@pFileId,--ARFileID = @TotalARFileID,
			...
			Begin
					...
					Select @PresentBillHeaderStatus = Status from BillHeader 
						...
						Begin
							Update BillHeader Set PaymentAmount = isnull(@TotalAmountPaid,0), PaymentReference = @TotalPaymentReference, 
								DateSettlement = @TotalPaymentDate , DenialCode = @TotalDenialCode, ARFileID = @pFileId,
								 Status =   CASE WHEN @TotalDenialCode is NULL OR @TotalDenialCode='' OR @TotalDenialCode=' ' 
											THEN [dbo].[GetBillNextStatus] (@PresentBillHeaderStatus,1) ELSE [dbo].[GetBillNextStatus] (@PresentBillHeaderStatus,2)  END
							Where BillHeaderID = @BillHeaderID_Previous and EncounterID = @TotalEncounterID and PatientID = @TotalPayFor;
						End
			...
			WHO: Shashank
			WHEN: 14 Aptil 2016
			WHat: Cursor to update the status of the claim with Claim level denials
			WHY: Code is missing for the claims having Claim level denial
			*/
			DECLARE cursorPIFix CURSOR fast_forward FOR  
			Select DIstinct PayDate,PayReference,PayFor,PayBy,
			PayBillID, PayEncounterID,PayFacilityID,PayCorporateID,PayXADenialCode,PayXAFileHeaderID,XActivityParsedId
			from Payment Where  PayStatus = 900 and PayAppliedStatus is NULL and PayType < 200
			And [PayXAFileHeaderID] = @pFileId
			Order by PayBillID ;
			
			DECLARE @cPayDate Datetime,  @cPayReference nvarchar(50), @cPayFor INT, @cPayBy INT,
			 @cCursorPayBillID INT,  @cPayEncounterID INT, @cPayFacilityID INT, @cPayCorporateID INT, @cPayXADenialCode nvarchar(50),
			 @cPayXAFileHeaderID bigint,@cXActivityParsedId nvarchar(50)
			
			OPEN cursorPIFix   
			FETCH NEXT FROM cursorPIFix INTO @cPayDate,@cPayReference,@cPayFor,@cPayBy,
			@cCursorPayBillID,@cPayEncounterID,@cPayFacilityID,@cPayCorporateID,@cPayXADenialCode,@cPayXAFileHeaderID,@cXActivityParsedId
			...
			BEGIN   
						...
						Set @TotalAmountPaid = 0
						Select @TotalAmountPaid = Sum(isnull(PaymentAmount,0)) from BillActivity 
						Where BillHeaderID = @cCursorPayBillID and EncounterID = @cPayEncounterID and FacilityId = @cPayFacilityID;
						...
						if @PresentBillHeaderStatus in (60,110,160)
						Begin
							Update BillHeader Set PaymentAmount = isnull(@TotalAmountPaid,0), PaymentReference = @cPayReference, 
								DateSettlement = @cPayDate , DenialCode = @cPayXADenialCode, ARFileID = @cPayXAFileHeaderID,
								 Status =   CASE WHEN @cPayXADenialCode is NULL OR @cPayXADenialCode='' OR @cPayXADenialCode=' ' 
											THEN [dbo].[GetBillNextStatus] (@PresentBillHeaderStatus,1) ELSE [dbo].[GetBillNextStatus] (@PresentBillHeaderStatus,2)  END
							Where BillHeaderID = @cCursorPayBillID and EncounterID = @cPayEncounterID and PatientID = @cPayFor;
						End
						ELSE
						Begin
							Update BillHeader Set PaymentAmount = isnull(@TotalAmountPaid,0), PaymentReference = @cPayReference, 
			...
			When: 01 April, 2016
			Who: Amit Jain
			Why: To update the XFileHeader status once Payments are applied in the XRemittanceAdvice and Payments Table.
			*/

			--******************************Changes start here*****************************************--
			Declare @FileStatus nvarchar(10),@FileStatustoSet nvarchar(5) ='1',
			@NonSystemClaims int=0,@SystemClaims int=0,@AllFileClaims int=0
			 ---- Non System claims
			Select @NonSystemClaims =  Count(1) from Payment Where [PayXAFileHeaderID] = @pFileId and PayStatus = 1000
			Select @SystemClaims =  Count(1) from Payment Where [PayXAFileHeaderID] = @pFileId and PayStatus <> 1000
			Select @AllFileClaims =  Count(1) from Payment Where [PayXAFileHeaderID] = @pFileId 
			Select @FileStatus = [Status] From XFileHeader Where FileId = @pFileId 

			If (@NonSystemClaims = @AllFileClaims)
				Update XFileHeader Set [Status] = 5  Where FileId = @pFileId
			else if (@NonSystemClaims <> 0 and @SystemClaims <> 0)
				Update XFileHeader Set [Status] = 4  Where FileId = @pFileId
			else 
				Update XFileHeader Set [Status] = 1  Where FileId = @pFileId
			--******************************Changes end here*****************************************--
