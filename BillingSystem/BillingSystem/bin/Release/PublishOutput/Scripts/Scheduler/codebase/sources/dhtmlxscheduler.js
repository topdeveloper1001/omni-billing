...
scheduler._click = {
    ...
    dhx_cal_next_button: function (dummy, step) {
        ...
        scheduler.setCurrentView(scheduler.date.add( //next line changes scheduler._date , but seems it has not side-effects
			scheduler.date[scheduler._mode + "_start"](new Date(scheduler._date)), (step || 1), scheduler._mode));// Himanshu 11Aug2015: Add new Date method for scheduler._date to get a valid date
    },
    ...
    buttons: {
        ...
        priority: function (id) {
            var c = GetGridResource("PriorityQueueMessage");// scheduler.locale.labels.confirm_deleting;
            scheduler._dhtmlx_confirm(c, scheduler.locale.labels.title_confirm_deleting, function () { scheduler.deleteEvent(id, undefined, true); });
            //scheduler._dhtmlx_confirm(c, scheduler.locale.labels.title_confirm_deleting, function () { scheduler.deleteEvent(id); });
        }
    }
};
...
scheduler._on_dbl_click = function (e, src) {
    ...
    switch (name) {
        ...
        case "dhx_month_body":
        case "dhx_marked_timespan": //added as we want to stop create event on mousedblclick for marked timespan
...
scheduler._reset_scale = function () {
    ...
    this._min_date = d;
    //this._els["dhx_cal_date"][0].innerHTML = '<input type="text" class="customDate pointer" readonly="readonly" onfocus="this.blur();" id="datepicker"><span>' + this.templates[this._mode + "_date"](dd, ed, this._mode) + '</span>';
    this._els["dhx_cal_date"][0].innerHTML = '<span>' + this.templates[this._mode + "_date"](dd, ed, this._mode) + '</span>';
    this._max_date = ed;
...
scheduler.config = {
    ...
    icons_select: ["icon_details", "icon_edit", "icon_delete", "icon_priority"],
...
scheduler.deleteEvent = function (id, silent, priority, deletingRecEvent) {
    if (deletingRecEvent != undefined) {
        this._loading = false;
    }
...
scheduler._render_v_bar = function (ev, x, y, w, h, style, contentA, contentB, bottom) {
    ...
    if (!bottom && scheduler.renderEvent(container, ev, w, h, contentA, contentB)) {
        ...
        // Himanshu 16-jul-2015: Split text and check if its undefined set empty value in dragged contract otherwise set contract detail
        var content = contentB.split('^');
        content1 = '';
        content2 = '';
        if (content[0]) {
            content1 = content[0];
        ...
        if (contentB.indexOf('^') > 0) {
            contentB = "<div class='productname-text'>" + content1 + "</div><div class='productname-text'>" + content2 + "</div>";
        }
        ...
        //+ '' + bg_color + '' -- remove background color for showing footer lines
        inner_html += '<div class="' + footer_class + '" style=" width:' + (w - 8) + 'px;' + (bottom ? ' margin-top:-1px;' : '') + color + '" ></div>';
...
scheduler.render_event_bar = function (ev) {
    ...
    var bg_color = (ev.color ? ("background:" + ev.color + ";") : "");
    var color = (ev.textColor ? ("color:" + ev.textColor + ";") : "#fff");
...
scheduler._fill_lightbox = function (id, box) {
    ...
    if (scheduler.templates.lightbox_header) {
        ...
        if (s[2].innerHTML.indexOf('^') > -1) {
            s[2].innerHTML = s[2].innerHTML.substring(0, s[2].innerHTML.indexOf('^'));
        }
    }
...
scheduler.save_lightbox = function () {
    ...
    //var saved = saveSchedulerEvent(data);
    ...
    if (!this._lightbox) {
        ...
        for (var i = 0; i < sns.length; i++) {
            ...
            if (this.config.wide_form) {
                html += "<div class='dhx_wrap_section'  id=lb_" + i + ">";
            }
            ...
            for (var i = 0; i < navline_divs.length; i++) {
                ...
                if (name) { // mode tab
                    ...
                    switch (name) {
                        case "timeline_tab":
                            div.style.left = "14px";
                            div.className += " dhx_cal_tab_standalone timeline_Button_Width";
                            break;
                        case "day_tab":
