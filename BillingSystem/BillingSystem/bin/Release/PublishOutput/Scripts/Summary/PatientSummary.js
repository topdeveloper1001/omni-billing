...
function DiagnosisOnReady() {
    ...
    SetGridSorting(BindDingnosisBySort, "#CurrentDiagnosisGrid");
    SetGridSorting(BindPhyFavBySort, "#PatientFavDiagnosisGrid");
    SetGridSorting(BindPreviousDiagnosisBySort, "#previousDiagnosisGrid");
}
...
function BindDingnosisBySort(event) {
    var url = summaryPageUrl + "BindEncounterDiagnosisListSorted";
    var encounterId = $("#hdCurrentEncounterId").val();
    var pid = $("#hdPatientId").val();
    if (event.data != null && (event.data.msg != null || event.data.msg != undefined || event.data.msg != "")) {
...
function BindPhyFavBySort(event) {
    var url = summaryPageUrl + "BindPhysicianFavDiagnosisListSorted";
    var encounterId = $("#hdCurrentEncounterId").val();
    //var pid = $("#hdPatientId").val();
    if (event.data != null && (event.data.msg != null || event.data.msg != undefined || event.data.msg != "")) {
        url += "?encounterId=" + encounterId + "&" + event.data.msg;
...
function BindPreviousDiagnosisBySort(event) {
    var url = summaryPageUrl + "BindPreviousEncounterDiagnosisListSorted";
    var encounterId = $("#hdCurrentEncounterId").val();
    var pid = $("#hdPatientId").val();
    if (event.data != null && (event.data.msg != null || event.data.msg != undefined || event.data.msg != "")) {
        url += "?encounterId=" + encounterId + "&patientId=" + pid + "&" + event.data.msg;
...
var BindAddOrdersPaneldata = function () {
    BindOrdersRelatedDropdowns(2);
    BindCategoriesInSummary("#ddlOrderTypeCategory", "#hdOrderTypeCategoryID");
    InitializeDateTimePicker();
    $("#ddlOrderStatus option[value='4']").remove();
...
//    if (confirm("Do you want to cancel the remaining task for the selected care plan task? ")) {
//        var jsonData = JSON.stringify({
//            id: id
//        });
//        $.ajax({
...
function AddOrder(id) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: summaryPageUrl + "AddPhysicianOrder",
        data: jsonData,
        dataType: "json",
        success: function (data) {
            var msg = "Records Saved successfully !";
