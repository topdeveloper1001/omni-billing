...
/****** Object:  StoredProcedure [dbo].[SprocSavePatientInfo]    Script Date: 4/25/2018 6:09:30 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN	
	...
	IF ISNULL(@pPassportNo,'')=''
		SET @pPassportNo=NULL
	-----------$$$$$$$$$$$$$$$$$$ QUERY $$$$$$$$$$$$$$$$$$$-------------------
	...
	BEGIN TRY;
		...
		Begin
			...
			BEGIN
				UPDATE PatientInfo
				SET PersonEmiratesIDNumber= ISNULL(@pSSN,PersonEmiratesIDNumber),PersonMedicalRecordNumber = ISNULL(@pMRN,PersonMedicalRecordNumber)
				,PersonFinancialNumber = @pFinanceNo,PersonMasterPatientNumber = @pMasterPatientNo
		...
		BEGIN
			...
			BEGIN
				...
				,D.IsDeleted=0,DocumentName=(Case WHEN 
		...
		BEGIN
			...
			IF EXISTS (SELECT 1 FROM PatientLoginDetail WHERE PatientID = @pPatientId AND ISNULL(IsDeleted,0)=0)
			BEGIN
				UPDATE PatientLoginDetail SET ModifiedBy = @pUserId, ModifiedDate = @pCurrentDate, IsDeleted=0,Email=@pEmail WHERE PatientID=@pPatientId
