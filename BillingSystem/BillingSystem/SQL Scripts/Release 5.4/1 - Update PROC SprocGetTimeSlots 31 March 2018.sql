...
BEGIN
	...
	From Scheduling Where Cast(ScheduleFrom as date)=@ConcernedDate And IsActive=1 And ISNULL(PhysicianId,'') !='' and [Status] NOT in (12,4)
	And PhysicianId NOT IN (
						Select DISTINCT S.PhysicianId From Scheduling S
						Where Cast(S.ScheduleFrom as date)=@ConcernedDate And TypeOfProcedure=12 And IsActive=1 and [Status]  
						NOT IN (12,4)--Type of Procedure is 12 for Holidays. 4 for Cancelled Booking
						   )
	...
	Begin
		...
		--OR (T1.TimeFrom = Cast(T2.TimeFrom as TIME) AND T1.TimeTo = Cast(T2.TimeTo as TIME))
			OR (T1.TimeFrom = Cast(T2.TimeFrom as TIME)
		OR T1.TimeTo = Cast(T2.TimeTo as TIME))
	--OR (T1.TimeTo = Cast(T2.TimeFrom as TIME)
	--	OR T1.TimeFrom = Cast(T2.TimeTo as TIME))
