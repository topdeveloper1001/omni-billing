IF OBJECT_ID('SprocImportBillingCodes','P') IS NOT NULL
	DROP PROCEDURE SprocImportBillingCodes

GO

/****** Object:  StoredProcedure [dbo].[SprocImportBillingCodes]    Script Date: 6/5/2018 6:41:37 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		From @TCodes T
	END
	Else If @pCodeType = 'POS'		--Place of Service
	BEGIN
		---Now fetch the Code data from the tables
		;With ExistingCodes (Id,EffectiveStartDate,EffectiveEndDate,Code,FacilityId)
			As
			(
				Select MAX(T.Id) As Id, EffectiveStartDate=MAX(T.EffectiveStartDate)
				,EffectiveEndDate=MAX(T.EffectiveEndDate),T.Code,FacilityId=MAX(T.FacilityId)
				From
					(
						Select C.Id, T.EffectiveFrom As EffectiveStartDate
						,T.EffectiveTill As EffectiveEndDate,C.Code,C.FacilityId 
						From PlaceOfService C 
						INNER JOIN @TCodes T ON C.Code = T.Code And C.FacilityId=@pFId And C.CorporateId=@pCId
						And ISNULL(C.IsActive,1)=1
					) T
				Group by T.Code
			)

		Update C SET C.EffectiveEndDate = (Case 
								When E.EffectiveStartDate IS NOT NULL THEN CAST(E.EffectiveStartDate as DATE) 
								ELSE @LocalDateTime - 1 END)
								,ModifiedDate = @LocalDateTime, ModifiedBy = @pUserId
		From PlaceOfService C
		INNER JOIN ExistingCodes E ON C.Id=E.Id
		
		INSERT INTO PlaceOfService
		([Name],Code,[Description],EffectiveStartDate,EffectiveEndDate
		,IsActive,CreatedBy,CreatedDate,FacilityId,CorporateId)
		Select T.OtherValue1,T.Code,T.CodeDescription
		,EffectiveStartDate=
		(
			Case When EffectiveFrom IS NOT NULL THEN CAST(EffectiveFrom as DATE)
			ELSE @LocalDateTime END
		)
		,EffectiveEndDate=
		(
			Case 
			WHEN EffectiveTill IS NOT NULL AND EffectiveFrom IS NOT NULL 
			AND CAST(EffectiveTill as date) > CAST(EffectiveFrom as date)
			THEN CAST(EffectiveTill as date)
			ELSE NULL END
		)
		,1,@pUserId,@LocalDateTime,@pFId,@pCId From @TCodes T
	END
	Else If @pCodeType = 'BM'		--Billing Modifier.
	BEGIN
		---Now fetch the Code data from the tables
		;With ExistingCodes (Id,EffectiveStartDate,EffectiveEndDate,Code,FacilityId)
			As
			(
				...
				From
					(
						Select B.Id, T.EffectiveFrom As EffectiveStartDate
						,T.EffectiveTill As EffectiveEndDate,B.Code,B.FacilityId 
						From BillingModifier B
						INNER JOIN @TCodes T ON B.Code = T.Code And B.FacilityId=@pFId
		...
		Update B
		SET B.EffectiveEndDate=
		(
			Case 
			When E.EffectiveStartDate IS NOT NULL THEN CAST(E.EffectiveStartDate as DATE) 
			ELSE @LocalDateTime - 1 END
		)
		,B.ModifiedDate = @LocalDateTime, B.ModifiedBy = @pUserId
		From BillingModifier B
		INNER JOIN ExistingCodes E ON B.Id=E.Id
		
		INSERT INTO BillingModifier
		([Name],[Type],Code,[Description],EffectiveStartDate,EffectiveEndDate
		,IsActive,CreatedBy,CreatedDate,FacilityId,CorporateId,IsFirst)
		Select T.OtherValue1,T.OtherValue2,T.Code,T.CodeDescription
		,EffectiveStartDate=
		(
			Case When EffectiveFrom IS NOT NULL THEN CAST(EffectiveFrom as DATE)
			ELSE @LocalDateTime END
		...
		,1,@pUserId,@LocalDateTime,@pFId,@pCId,0 From @TCodes T
	END
END
GO
