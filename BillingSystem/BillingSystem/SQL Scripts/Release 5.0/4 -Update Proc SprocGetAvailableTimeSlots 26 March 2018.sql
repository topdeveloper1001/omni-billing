...
/****** Object:  StoredProcedure [dbo].[SprocGetAvailableTimeSlots]    Script Date: 3/26/2018 9:13:07 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	Update T Set T.OpeningTime =case when convert(char(5), DT.OpeningTime,108)  >= convert(char(5), @LocalDateTime, 108)
	 then   CONVERT(VARCHAR(5),CONVERT(DATETIME,DT.OpeningTime, 0), 108) else  convert(char(5),  @LocalDateTime, 108) END,
	...
	BEGIN
		Update @PhysiciansData Set OpeningTime = case when convert(char(5), FR.FromDate,108)  >= convert(char(5),  @LocalDateTime, 108)
	 then   CONVERT(VARCHAR(5),CONVERT(DATETIME,FR.FromDate, 0), 108) else  convert(char(5),  @LocalDateTime, 108) END ,
