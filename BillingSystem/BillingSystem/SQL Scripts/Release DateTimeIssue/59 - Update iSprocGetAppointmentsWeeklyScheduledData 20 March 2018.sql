IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'iSprocGetAppointmentsWeeklyScheduledData')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE iSprocGetAppointmentsWeeklyScheduledData
GO

/****** Object:  StoredProcedure [dbo].[iSprocGetAppointmentsWeeklyScheduledData]    Script Date: 21-03-2018 11:23:59 ******/
SET ANSI_NULLS ON
GO
...
--Exec iSprocGetAppointmentsWeeklyScheduledData 12,8
...
Begin
	...
	Declare @EDate datetime = dateadd(day, 7-datepart(dw, @LocalDateTime), CONVERT(date,@LocalDateTime)) 
	...
	(
		Select DISTINCT datediff(Minute, S.ScheduleFrom, S.ScheduleTo) AS TotalTimeInMinutes
