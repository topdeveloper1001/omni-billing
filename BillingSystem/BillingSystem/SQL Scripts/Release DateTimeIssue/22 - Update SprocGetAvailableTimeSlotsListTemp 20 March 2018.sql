IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SprocGetAvailableTimeSlotsListTemp')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
         DROP PROCEDURE SprocGetAvailableTimeSlotsListTemp
GO

/****** Object:  StoredProcedure [dbo].[SprocGetAvailableTimeSlotsListTemp]    Script Date: 20-03-2018 17:29:17 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[SprocGetAvailableTimeSlotsListTemp] --[dbo].[SPROC_GetAvailableTimeSlotsList_KP] '8/31/16 12:00:00 AM','8/31/16 12:00:00 AM',1,8,1054
(
...
BEGIN
	...
	Declare @TempDeps Table(DepId int, OpeningTime nvarchar(10), ClosingTime nvarchar(10))
...
Declare @TimeSlotTable table([ID] [int] IDENTITY(1,1),DateTimeInterval datetime)
...
Declare @OccupiedTimeSlotsTable1 table(OccupiedTimeSlot1 nvarchar(50),OccupiedTimeSlot2 nvarchar(50))
...
Select DISTINCT DepId,OpeningTime,ClosingTime From @TempClinicians
...
Declare @PId bigint,@Cnt int=0

Declare @Tc Table(CId bigint)

INSERT INTO @Tc
Select DISTINCT PhyId From @TempClinicians

While Exists (Select 1 From @Tc)
Begin
	Select TOP 1 @PId = CId From @Tc

	Select * From Scheduling Where AssociatedId=@PId And ScheduleFrom >= @LocalDateTime

	DECLARE @dtStart AS DATETIME =@pFromDate --'20151028 00:00:00'
		,@dtEnd AS DATETIME = @pToDate --'20151028 23:59:00'
		,@iInterval AS INT = @timeDiffernece--60;  --30 min interval
...
--- Insert the data from the Scheduling table to the temp table (Occupied Time slots)
	...
	Select CONVERT(VARCHAR(5),CONVERT(DATETIME,ScheduleFrom, 0), 108),
	CONVERT(VARCHAR(5),CONVERT(DATETIME,ScheduleTo, 0), 108) FROM Scheduling 
	where
	(PhysicianId IN (SELECT DISTINCT PhyId from @TempClinicians) OR (AssociatedId  in (select PhyId from @TempClinicians) 
...
--- Table to retuen values (TIme slots that are avialable and are in between the department opening and closing time)
	---- Vacation check
	...
	BEGIN
		 ...
		 Exists(Select 1 from Scheduling Where FacilityId = @pFacilityId and TypeOfProcedure = 12 and ExtValue2 is not null and Cast(ScheduleFrom as date) = Cast(@pFromDate as Date))
		 begin	
			Set @IsHoliday=1
		 end
	END
...
--IF Exists (Select 1 from FacilityStructure where GlobalCodeID = 84 and ISNULL(ExternalValue4,'')
--like '%'+ CAST(@pAppointMentType as nvarchar(10))+'%' and Facilityid = @pFacilityId)
	...
	IF EXISTS (Select 1 From FacilityStructure S
	 ...
	 And Ap.Id = @pAppointMentType And ISNULL(ExternalValue4,'') != '')
	BEGIN
...
-- If not holiday then go to the logic
	...
	BEGIN
		...
		BEGIN
			While @DepCount > 0
			Begin
				Select TOP 1 @CurrentDepId = DepId, @Dept_OpeningTime = OpeningTime, @Dept_ClosingTime = ClosingTime From @TempDeps
	...
	BEGIN
		...
		BEGIN
			Select DISTINCT *,STUFF((SELECT ',' + CAST(OpeningDayId AS VARCHAR(MAX)) From DeptTimming 
			Where FacilityStructureID = DeptId FOR XML PATH('')),1,1,'') DepOpeningDays 
