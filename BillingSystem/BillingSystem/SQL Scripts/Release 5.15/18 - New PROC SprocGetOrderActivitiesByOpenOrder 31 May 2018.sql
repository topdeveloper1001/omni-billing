 IF EXISTS ( SELECT *
            FROM   sysobjects
            WHERE  id = object_id(N'SprocGetOrderActivitiesByOpenOrder')
                   and OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
    DROP PROCEDURE SprocGetOrderActivitiesByOpenOrder
END
GO
 
Create PROCEDURE [dbo].[SprocGetOrderActivitiesByOpenOrder]  
(  
@pOpenOrderId bigint  
)  
AS  
...
DECLARE @pEncounterId BIGINT=( Select Top 1 EncounterId from OpenOrder O WHere O.OpenOrderId=@pOpenOrderId)
 Declare @pOrderTypeGCC nvarchar(50) = '1201'
 ...
 --Get All Order Activities by OpenOrder ID And Insert int Temp Table  
 Insert InTo @Table  
 ...
 (Select TOP 1 GlobalCodeName From GlobalCodes Where GlobalCodeValue = Cast(OrderType As Varchar) And GlobalCodeCategoryValue = @pOrderTypeGCC and isdeleted=0) OrderTypeName  
 ...
 From OrderActivity   
 Where OrderID = @pOpenOrderId AND ISNULL(IsActive,1)=1
