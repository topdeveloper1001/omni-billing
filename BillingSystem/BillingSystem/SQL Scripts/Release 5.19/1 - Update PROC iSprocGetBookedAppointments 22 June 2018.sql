...
BEGIN 
	...
	Begin
		...
		IF @pPatientId > 0 AND ISNULL(@pOnlyUpcoming,0)=0
			SELECT @MinDate=MIN(ScheduleFrom) From Scheduling Where AssociatedId=@pPatientId
		...
		And 
		(
			---WHY Add 60 Here: To Get the Appointments for the next 60 days
			S.ScheduleFrom BETWEEN IIF(ISNULL(@pOnlyUpcoming,0)=0,@MinDate,@LocalDateTime) 
									AND IIF(ISNULL(@pOnlyUpcoming,0)=0,@LocalDateTime,@LocalDateTime+60) 
		)
		AND 
		(
			@pForToday=0
